...
import { Plus, Play, Pause, Calendar, ArrowRightLeft, PowerOff } from 'lucide-react';
...
export default function RecurringPage() {
  ...
  const onSubmit = (d: any) => {
    const payload: any = {
      ...
      next_due_date: d.start_date, // set next due to start date initially
    };

    if (d.category_id) {
      payload.category_id = parseInt(d.category_id);
    }
    if (d.frequency === 'monthly' && d.scheduled_day) {
      payload.scheduled_day = parseInt(d.scheduled_day);
    } else {
      delete payload.scheduled_day;
    }
    if (!d.end_date) {
      ...
      <div className="flex items-center justify-between">
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent className="bg-slate-900 border-slate-800 text-white w-full sm:max-w-md overflow-y-auto">
            <SheetHeader>
              <SheetTitle className="text-white">Create Recurring Template</SheetTitle>
            </SheetHeader>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-6">
              ...
              <div className="space-y-1.5">
                ...
                <Input {...register('name', { required: true })} placeholder="e.g. TNB Electricity Bill"
                  className="bg-slate-800 border-slate-700 text-white" />
              </div>

              <div className="space-y-1.5">
                <Label className="text-slate-300">Default Amount (RM)</Label>
                <Input {...register('default_amount', { required: true })} type="number" step="0.01" placeholder="0.00"
                  className="bg-slate-800 border-slate-700 text-white" />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Controller name="account_id" control={control} rules={{ required: true }} render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value?.toString()}>
                    ...
                    <SelectContent className="bg-slate-800 border-slate-700 text-white">
                      {accounts.map((a: any) => (
                        <SelectItem key={a.id} value={a.id.toString()}>{a.nickname}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                )} />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {frequency === 'monthly' && (
                  <div className="space-y-1.5">
                    ...
                    <Input {...register('scheduled_day', { min: 1, max: 31 })} type="number" placeholder="e.g. 25"
                      className="bg-slate-800 border-slate-700 text-white" />
                  </div>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Textarea {...register('remarks')} placeholder="Add notes here..."
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="outline" className={`text-[10px] uppercase font-semibold border-slate-700 ${
                        ...
                        t.status === 'paused' ? 'text-yellow-400 border-yellow-500/20 bg-yellow-500/5' :
                        'text-slate-500'
                      }`}>
                        {t.status}
                      </Badge>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                  ...
                  {t.status === 'active' ? (
                    <button
                      ...
                      className="p-1.5 rounded-lg border border-slate-800 bg-slate-900 text-slate-400 hover:text-yellow-400 hover:border-yellow-500/30 transition-all"
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg border border-slate-800 bg-slate-900 text-slate-400 hover:text-green-400 hover:border-green-500/30 transition-all"
                  ...
                  {t.status !== 'ended' && (
                    <button
                      onClick={() => {
                        if (confirm('Are you sure you want to end this recurring template?')) {
